"use client";

import { useState } from "react";
import { Send, CheckCircle, Upload, AlertCircle } from "lucide-react";
import { readJsonResponse } from "@/lib/http-json";

type FormState = "idle" | "loading" | "success" | "error";
type FieldErrors = Record<string, string>;

function getFieldErrors(value: unknown): FieldErrors {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>)
      .map(([key, message]) => [
        key,
        typeof message === "string"
          ? message
          : Array.isArray(message)
            ? String(message[0] || "")
            : "",
      ])
      .filter(([, message]) => Boolean(message)),
  ) as FieldErrors;
}

function FieldError({ message }: { message?: string }) {
  return message ? (
    <p className="mt-1.5 text-xs font-bold text-urgent">{message}</p>
  ) : null;
}

export default function WriteForm() {
  const [state, setState] = useState<FormState>("idle");
  const [message, setMessage] = useState("");
  const [fieldErrors, setFieldErrors] = useState<FieldErrors>({});
  const [publishWithName, setPublishWithName] = useState(true);

  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (state === "loading") return;

    setState("loading");
    setMessage("");
    setFieldErrors({});

    const form = e.currentTarget;
    const formData = new FormData(form);
    formData.set("allowPublish", String(publishWithName));

    try {
      const res = await fetch("/api/submissions/article", {
        method: "POST",
        body: formData,
      });
      const data = await readJsonResponse(res);
      const errors = getFieldErrors(data.errors);

      if (!res.ok || !data.ok) {
        setFieldErrors(errors);
        throw new Error(
          Object.values(errors)[0] ||
            data.message ||
            "تعذر إرسال المقال حاليًا. يرجى المحاولة لاحقًا.",
        );
      }

      setState("success");
      form.reset();
      setPublishWithName(true);
      setFieldErrors({});
    } catch (error) {
      setState("error");
      setMessage(error instanceof Error ? error.message : "حدث خطأ غير متوقع");
    }
  }

  if (state === "success") {
    return (
      <div className="bg-white rounded-2xl shadow-card p-10 text-center">
        <div className="w-16 h-16 bg-hope/10 rounded-full flex items-center justify-center mx-auto mb-5">
          <CheckCircle size={32} className="text-hope" />
        </div>
        <h3 className="text-xl font-bold font-cairo text-navy mb-3">
          تم إرسال المقال بنجاح
        </h3>
        <p className="text-text-light font-tajawal text-sm leading-relaxed">
          سنراجعه قبل النشر، وسنتواصل معك عند الحاجة إلى أي توضيح.
        </p>
      </div>
    );
  }

  return (
    <div className="bg-white rounded-2xl shadow-card p-7 md:p-9">
      <div className="mb-7">
        <h2 className="text-xl font-bold font-cairo text-navy mb-2">
          نموذج إرسال مقال
        </h2>
        <p className="text-text-muted text-sm font-tajawal">
          نرحب بأي مقال يلامس الإنسان والمجتمع والمعرفة، ويتسم بالموضوعية
          والأمانة.
        </p>
      </div>

      {state === "error" ? (
        <div className="mb-5 flex items-center gap-2 rounded-xl bg-urgent/10 p-4 text-sm font-bold text-urgent">
          <AlertCircle size={18} /> {message}
        </div>
      ) : null}

      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        <div className="bg-ivory rounded-xl p-5 space-y-4">
          <h3 className="text-sm font-bold font-cairo text-navy border-b border-ivory-dark pb-2">
            معلومات الكاتب
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="label-field">
                الاسم الكامل <span className="text-urgent">*</span>
              </label>
              <input
                name="fullName"
                type="text"
                className="input-field"
                required
                minLength={2}
              />
              <FieldError message={fieldErrors.fullName} />
            </div>
            <div>
              <label className="label-field">
                البريد الإلكتروني <span className="text-urgent">*</span>
              </label>
              <input
                name="email"
                type="email"
                className="input-field ltr"
                dir="ltr"
                required
              />
              <FieldError message={fieldErrors.email} />
            </div>
          </div>
          <div>
            <label className="label-field">رقم واتساب</label>
            <input
              name="phone"
              type="tel"
              className="input-field ltr"
              dir="ltr"
            />
            <FieldError message={fieldErrors.phone} />
          </div>
          <div>
            <label className="label-field">
              نبذة قصيرة عن الكاتب{" "}
              <span className="text-text-muted font-normal">اختياري</span>
            </label>
            <textarea
              name="authorBio"
              className="textarea-field min-h-[80px]"
              rows={3}
            />
            <FieldError message={fieldErrors.authorBio} />
          </div>
          <div>
            <label className="label-field">
              صورة الكاتب{" "}
              <span className="text-text-muted font-normal">اختياري</span>
            </label>
            <label className="block border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:border-gold/40 transition-colors cursor-pointer focus-within:border-gold focus-within:ring-2 focus-within:ring-gold/20">
              <Upload size={18} className="text-gold mx-auto mb-1.5" />
              <p className="text-xs text-text-muted font-tajawal">
                اضغط لاختيار صورة شخصية
              </p>
              <input
                name="avatar"
                type="file"
                className="sr-only"
                accept="image/*"
              />
            </label>
          </div>
          <div>
            <label className="label-field">رابط اجتماعي</label>
            <input
              name="socialUrl"
              type="url"
              className="input-field ltr text-xs"
              dir="ltr"
              placeholder="https://"
            />
            <FieldError message={fieldErrors.socialUrl} />
          </div>
        </div>

        <div className="bg-ivory rounded-xl p-5 space-y-4">
          <h3 className="text-sm font-bold font-cairo text-navy border-b border-ivory-dark pb-2">
            معلومات المقال
          </h3>
          <div>
            <label className="label-field">
              عنوان المقال <span className="text-urgent">*</span>
            </label>
            <input
              name="title"
              type="text"
              className="input-field"
              required
              minLength={5}
            />
            <FieldError message={fieldErrors.title} />
          </div>
          <div>
            <label className="label-field">ملخص المقال</label>
            <textarea
              name="summary"
              className="textarea-field min-h-[80px]"
              rows={3}
            />
            <FieldError message={fieldErrors.summary} />
          </div>
          <div>
            <label className="label-field">
              نص المقال <span className="text-urgent">*</span>
            </label>
            <textarea
              name="body"
              className="textarea-field"
              required
              minLength={20}
              rows={12}
            />
            <FieldError message={fieldErrors.body} />
          </div>
          <div>
            <label className="label-field">صورة غلاف</label>
            <label className="block border-2 border-dashed border-gray-200 rounded-xl p-5 text-center hover:border-gold/40 transition-colors cursor-pointer focus-within:border-gold focus-within:ring-2 focus-within:ring-gold/20">
              <Upload size={20} className="text-gold mx-auto mb-2" />
              <p className="text-sm text-text-muted font-tajawal">
                اضغط لاختيار صورة الغلاف
              </p>
              <input
                name="coverImage"
                type="file"
                className="sr-only"
                accept="image/*"
              />
            </label>
          </div>
          <div>
            <label className="label-field">مرفقات إضافية</label>
            <label className="block border-2 border-dashed border-gray-200 rounded-xl p-5 text-center hover:border-gold/40 transition-colors cursor-pointer">
              <Upload size={20} className="text-gold mx-auto mb-2" />
              <p className="text-sm text-text-muted font-tajawal">
                صور، PDF، صوت أو فيديو
              </p>
              <input
                name="attachments"
                type="file"
                className="sr-only"
                multiple
                accept="image/*,.pdf,audio/*,video/*"
              />
            </label>
          </div>
        </div>

        <div className="bg-gold/5 border border-gold/20 rounded-xl p-4">
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              name="allowPublish"
              type="checkbox"
              checked={publishWithName}
              onChange={(e) => setPublishWithName(e.target.checked)}
              className="mt-0.5 w-4 h-4 accent-gold"
            />
            <span className="text-sm font-semibold font-cairo text-navy">
              أريد نشر المقال باسمي الكامل
            </span>
          </label>
        </div>

        <button
          disabled={state === "loading"}
          type="submit"
          className="w-full btn-hope justify-center py-3.5 text-base disabled:opacity-60"
        >
          <Send size={18} />{" "}
          {state === "loading" ? "جارٍ الإرسال..." : "إرسال المقال"}
        </button>
      </form>
    </div>
  );
}
