...
async function saveSettings(formData: FormData) {
  "use server";
  const values = {
    siteName: String(formData.get("siteName") || "مدى الناس"),
  ...
  return (
    <AdminSection title="إعدادات الموقع" description="إعدادات محفوظة في قاعدة البيانات وليست حقولًا شكلية.">
      <form action={saveSettings} className="grid gap-4 md:grid-cols-2">
        <label className="block"><span className="text-sm font-bold">اسم الموقع</span><input name="siteName" className="mt-2 w-full rounded-2xl border border-navy/10 bg-ivory-light p-3" defaultValue={get("siteName", "مدى الناس")} /></label>
        ...
        <label className="block md:col-span-2"><span className="text-sm font-bold">وصف SEO العام</span><textarea name="seoDescription" className="mt-2 min-h-28 w-full rounded-2xl border border-navy/10 bg-ivory-light p-3" defaultValue={get("seoDescription", "منصة إنسانية عربية مستقلة تنقل قصص الناس وقضاياهم بكرامة ووضوح.")} /></label>
